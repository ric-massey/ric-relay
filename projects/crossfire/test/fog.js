...
function hud() {
  ...
  const H = sandbox.window.CrossfireSurveyHUD;
  assert.ok(H, "survey-hud.js did not publish CrossfireSurveyHUD");
...
console.log("CROSSFIRE chart checks");
...
if (fails) {
  console.error("\nCROSSFIRE chart checks FAILED (" + fails + ")");
  process.exit(1);
}
console.log("CROSSFIRE chart checks passed");
