...
**What got built, in the order it was built, and how each piece actually
landed.** This is not a plan and nothing in it is waiting to be done. What is
left is [`SURVEY-PLAN.md`](SURVEY-PLAN.md), which is short because this file
exists.

Survey started as a quiet mode with no enemies and no losing. It is a survival
exploration game now, and this is the trail of how that happened: eight phases
finished outright, four with a single item still out, every session at the stick
that turned something up, and every bug that was worth writing down.

## How to read it

1. **The head is the newest.** "Who holds the sky" is the most recent work, not
   the next; it was written at the top because that is where a reader starts.
2. **Then the record, oldest first:** what already existed, the bugs found since,
   then Phase 1, the places, Phase 2, the screen, Phase 3, Phase 4, the pages,
   Phase 5.
3. **Then Phase 7, and then Phase 6** — in that order, on purpose. The interface
   was built before the simulation phase because a mode you cannot read is a mode
   nobody can judge.
4. **A section with a date and no number is a session**, not a plan item — "Four
   things from the cockpit", "A long list from the cockpit", the bug sweeps. They
   are what flying it turned up, kept because the reasons are in them.
5. **A status is on the heading.** `**DONE**` means built, tested and pushed. A
   heading with no status is a session or a piece of reasoning.

Two numbering scars, left rather than renumbered so old notes still resolve:
**there is no 7.7**, and 5.5 sits after 5.6 because 5.6 was built first. The 7.x
sections also appear 7.1–7.6, 7.13–7.15, then 7.8–7.12.

**Superseded, not deleted.** Three passages describe things that were later
replaced — the danger bands, and waypoints. They carry a supersede note where
they stand, because every number tuned against them is still in the game and
this is where those numbers came from. Nothing here gets removed; see the rule
below.
...
*Moved to [`SURVEY-PLAN.md`](SURVEY-PLAN.md) — it is not built, so it belongs with what is left. It will come back here when it lands.*
...
> **This list is the order as it was set, not the state as it is.** Three of the
> four have closed since; each one says so under it, and
> [`SURVEY-PLAN.md`](SURVEY-PLAN.md) is the live list. Kept because the *order*
> was a decision and the reasons for it are here.

1. **6.6 — authored mysteries.** Nine places where the brief asks for dozens, and
...
3. **6.5 — consequences that persist.** Three kinds of memory so far. The cheap
   ...
   > **Done.** All three of those, plus memorials on one battle in three and
   > reputation that persists per power — see the 6.5 row in *Where Phase 6
   > actually stands*.
4. **6.3's last gap** — NPCs cannot run out of anything and a well does not kill
   ...
   > **Done.** Ships run out of water when something goes wrong for them, and a
   > drifting hauler is a situation you can spend your own tank on or strip four
   > minutes later; traffic carries a well's drift, steers round one when it has
   > the attention to spare, and is swallowed if it loses. Both are in the 6.3
   > row of the same table.

So of the four, only **6.6** is still open, and it is item 1 in
[`SURVEY-PLAN.md`](SURVEY-PLAN.md).
