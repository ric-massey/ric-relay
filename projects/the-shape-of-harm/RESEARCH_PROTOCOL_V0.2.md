# The Shape of Harm Research Framework — Protocol Draft v0.2

**Date:** 23 July 2026  
**Status:** Four pilot estimands are provisionally frozen for feasibility and content-validity review. The protocol is not preregistered, estimated, scored, validated, or suitable for policy/clinical decisions.
...
### 3.4 Estimand record and version control

Each published question must specify the following attributes before evidence searching begins:

1. target population;
2. exposure strategy or condition;
3. comparator strategy or reference state;
4. outcome variable or outcome set;
5. strategy for post-exposure or intercurrent events;
6. population-level summary measure;
7. time horizon;
8. geography and calendar period;
9. comparability class;
10. identification assumptions and minimum data requirements.

The structure adapts the ICH E9(R1) estimand framework to observational and public-health risk questions. Drug-risk scenarios additionally register supply source, formulation, route, setting, supervision, co-use, and vulnerability because those features can change the target quantity.

The four pilot questions are frozen in `PILOT_ESTIMANDS_V0.2.md`, `estimand-registry.csv`, and `estimands-v0.2.json`. “Provisionally frozen” means feasibility work must use the registered wording. Material changes create a new version and dated amendment; they are not silently edited after evidence is reviewed.

The four pilot estimands belong to different comparability classes and must not be combined into a single rank. They are method-development anchors, not four interchangeable drug rows.
...
### Frozen author-draft estimands

Four exact pilot estimands are now provisionally frozen:

1. **EST-AE-ALC-001:** 24-hour acute outcomes after a 60 g regulated alcohol episode in U.S. community settings.
2. **EST-RU-TOB-001:** 10-year continuation of 20 cigarettes/day versus complete cessation among baseline daily smokers.
3. **EST-AE-OPI-001:** 24-hour severe toxicity after an intranasal episode involving an unverified product sold as an opioid in the 2024 U.S. illicit market.
4. **EST-AE-PSI-001:** 7-day serious and severe adverse outcomes after controlled 25 mg oral synthetic psilocybin versus placebo among trial-eligible adults with major depressive disorder.

Full definitions, interpretation boundaries, and feasibility judgments are in `PILOT_ESTIMANDS_V0.2.md` and `estimand-registry.csv`.

These rows are not intended to be ranked together. They test four methodological problems: episode denominators, target-trial emulation, changing illicit supply, and controlled-administration safety synthesis.

### Immediate next gate: independent content-validity review

Reviewers use `estimand-content-validity-form.csv` to rate population relevance, exposure and comparator precision, outcome completeness and overlap, time windows, handling of intercurrent events, summary-measure interpretability, omitted contexts, and practical estimability. Material revisions increment the estimand version before preregistration or evidence extraction.

### Feasibility sequencing

- Begin empirical extraction with **EST-AE-PSI-001** and **EST-RU-TOB-001**, which have the clearest data pathways.
- Conduct denominator-feasibility studies before estimating **EST-AE-ALC-001** or **EST-AE-OPI-001**.
- Publish “not estimable from available data” where numerator and denominator cannot be made compatible; do not substitute expert midpoints.
- Use duplicate review and extraction, freeze analysis before held-out validation, and publish all code, forms, decisions, deviations, and null results.
