...
try {
  ...
  console.log("KONDRITE browser checks SKIPPED — playwright is not installed");
...
async function main() {
  ...
  const mods = await page.evaluate(() => ({
    world: typeof window.KondriteSurveyWorld,
    save:  typeof window.KondriteSurveySave,
    hud:   typeof window.KondriteSurveyHUD,
    menu:  typeof window.KondriteMenu,
  ...
  const scrolled = await page.evaluate(async () => {
    const cf = window.__cf, H = window.KondriteSurveyHUD;
  ...
  const dragged = await page.evaluate(async () => {
    const cf = window.__cf, H = window.KondriteSurveyHUD, s = cf.survey();
    const c = document.getElementById("game");
    const r = c.getBoundingClientRect();
...
main().then(() => {
  if (fails) {
    console.error("\nKONDRITE browser checks FAILED (" + fails + ")");
    process.exit(1);
  }
  console.log("KONDRITE browser checks passed");
}).catch(e => {
  console.error("KONDRITE browser checks THREW\n", e);
  process.exit(1);
});
