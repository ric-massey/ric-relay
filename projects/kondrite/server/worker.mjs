/* KONDRITE — room service on Cloudflare
   ────────────────────────────────────────────────────────────────────────────
   ...
   instance, one place, one copy of the list. `idFromName("kondrite")` names
...
export default {
  fetch(request, env) {
    return env.ROOMS.get(env.ROOMS.idFromName("kondrite")).fetch(request);
  }
};
