...
/* KONDRITE — WHO IS FLYING IT
   ...
   turned into somewhere to be, and what is lying about. surveyTraffic is the
   spine: what a ship notices, where it wants to be and how it steers there
   are here; what happens to it and what it fires are in traffic.js.
...
function surveyTraffic(dt) {
  ...
  for (let i = surv.traffic.length - 1; i >= 0; i--) {
    ...
    if (trafficSwallowed(t, i, took)) continue;

    if (trafficAdrift(t, dt, i, me)) continue;

    if (trafficDistress(t, dt, i, me)) continue;
    ...
    if (trafficStunned(t, dt)) continue;
    ...
    trafficBump(t, i, me, near);

    if (trafficFire(t, dt, me, armed, fake, skill)) continue;
  }
}
