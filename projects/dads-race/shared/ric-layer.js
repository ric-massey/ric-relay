/* Ric's layer — runs on top of Victoria's app, for Ric and Sydney only.
 *
 * Victoria's app (shared/original/, imported unchanged from her index.html) loads first
 * and is the whole app. This file then adds Ric's race-day screens and replaces a handful
 * of her functions with Ric's versions. shared/bootstrap.js loads the two in that order and
 * holds her start-up until this file has run.
 *
...
function usesMissionShell(){ return !!(ME && MISSION_PROFILES.includes(String(ME.name||'').toLowerCase())); }
...
function applyRicShell(){
  ...
  // Notes: Ric's message feed and race-update filters (renderNotes above is Ric's too).
