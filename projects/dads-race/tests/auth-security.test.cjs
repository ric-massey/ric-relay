...
test('direct race pages require a session and canonical authenticated profile', () => {
  ...
  const app = (read('shared/data.js') + read('shared/original/app.js') + read('shared/notes-layer.js') + read('shared/ric-layer.js'));
...
test('with no key the app runs the made-up demo, never the live database', () => {
  ...
  assert.match((read('shared/data.js') + read('shared/original/app.js') + read('shared/notes-layer.js') + read('shared/ric-layer.js')), /window\.HermiscusDemo\.build\(Date\.now\(\)\)/);
});
...
test('only Ric and Sydney get the modified app; everyone else gets the original', () => {
  ...
  assert.match(read('shared/notes-layer.js'), /const MISSION_PROFILES = \['ric','sydney'\];/);
